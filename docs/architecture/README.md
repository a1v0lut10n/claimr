...
| Name      | What it is |
|-----------|------------|
| `parser`  | `src/parser/` — LR parser generated by rustemo from `claimr.rustemo` at build time (`build.rs`), with semantic actions (`claimr_actions.rs`) that build the AST in `src/ast.rs`; public entry points `parse_program` / `parse_clause` in `src/lib.rs`, errors as `ParseError` with line/column. Decision: [`../design/2026-08-17-rustemo-parser-generator-over-nom.md`](../design/2026-08-17-rustemo-parser-generator-over-nom.md). |
