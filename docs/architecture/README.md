...
| `cli`     | `src/main.rs` + `src/repl.rs` — the `claimr` binary: `claimr file` runs a program's `?-` queries (`--parse` dumps the AST, `--limit N` caps answers); `claimr` / `claimr -i file` is the interactive loop (Prolog-style stepping, clauses at the prompt extend the session, `:` commands, Ctrl-C). GCC-style `file:line:col:` diagnostics. Design: [`../design/2026-08-18-repl-interaction-model.md`](../design/2026-08-18-repl-interaction-model.md). |
